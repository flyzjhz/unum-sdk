...
static int wt_print_cipher(char *buf, const uint8_t *data)
{
    int buf_offset = 0;
    ...
    if (memcmp(data, ms_oui, 3) == 0) {
        switch (data[3]) {
        case 0:
            buf_offset += wt_print_string(buf, ENC_BUF_SIZE, buf_offset, 
...
// Returns the number of bytes dumped into buf
static int wt_print_auth(char *buf, const uint8_t *data)
{
    int buf_offset = 0;
    ...
    if (memcmp(data, ms_oui, 3) == 0) {
        switch (data[3]) {
        case 1:
            buf_offset += sprintf(buf + buf_offset, "IEEE 802.1X");
            break;
        case 2:
            buf_offset += sprintf(buf + buf_offset, "PSK");
            break;
        default:
            buf_offset += sprintf(buf + buf_offset, "%.02x-%.02x-%.02x:%d",
        ...
        case 2:
            ...
            break;
        case 3:
            buf_offset += sprintf(buf + buf_offset, "FT/IEEE 802.1X");
            break;
        case 4:
            buf_offset += sprintf(buf + buf_offset, "FT/PSK");
            break;
        case 5:
            buf_offset += sprintf(buf + buf_offset, "IEEE 802.1X/SHA-256");
            break;
        case 6:
            buf_offset += sprintf(buf + buf_offset, "PSK/SHA-256");
            break;
        case 7:
            buf_offset += sprintf(buf + buf_offset, "TDLS/TPK");
            break;
        case 8:
            buf_offset += sprintf(buf + buf_offset, "SAE");
            break;
        case 9:
            buf_offset += sprintf(buf + buf_offset, "FT/SAE");
            break;
        case 11:
            buf_offset += sprintf(buf + buf_offset, "IEEE 802.1X/SUITE-B");
            break;
        case 12:
            buf_offset += sprintf(buf + buf_offset, "IEEE 802.1X/SUITE-B-192");
            break;
        case 13:
            buf_offset += sprintf(buf + buf_offset, "FT/IEEE 802.1X/SHA-384");
            break;
        case 14:
            buf_offset += sprintf(buf + buf_offset, "FILS/SHA-256");
            break;
        case 15:
            buf_offset += sprintf(buf + buf_offset, "FILS/SHA-384");
            break;
        case 16:
            buf_offset += sprintf(buf + buf_offset, "FT/FILS/SHA-256");
            break;
        case 17:
            buf_offset += sprintf(buf + buf_offset, "FT/FILS/SHA-384");
            break;
        case 18:
            buf_offset += sprintf(buf + buf_offset, "OWE");
            break;
        default:
            buf_offset += sprintf(buf + buf_offset, "%.02x-%.02x-%.02x:%d",
                                data[0], data[1] ,data[2], data[3]);
...
static void wt_print_rsn_ie(char *buf, const char *defcipher,
                        const char *defauth, uint8_t len, const uint8_t *data)
{
    ...
    if (len < 4) {
        // Too small. Skip
        buf_offset = sprintf(buf + buf_offset, "Group : %s ", defcipher);
        buf_offset += sprintf(buf + buf_offset, "Pairwise : %s", defcipher);
        return;
    }

    // Group cipher, followed by Pairwise, followed by Auth algorithm
    // Start with Group Cipher
    buf_offset += sprintf(buf + buf_offset, "Group : ");
    buf_offset += wt_print_cipher(buf + buf_offset, data);
    ...
    if (len < 2) {
        // Too small. Skip
        buf_offset += sprintf(buf + buf_offset, ", Pairwise: %s ",
                                                                defcipher);
        return;
    }
    ...
    // Pairwise Cipher
    buf_offset += sprintf(buf + buf_offset, ", Pairwise: ");
    for (i = 0; i < count; i++) {
        if (i != 0) {
            // Add a space between each supported cipher
            buf_offset += sprintf(buf + buf_offset, " ");
        }
        buf_offset += wt_print_cipher(buf + buf_offset, data + 2 + (i * 4));
    }
    ...
    if (len < 2) {
        // Too small. Skip
        buf_offset += sprintf(buf + buf_offset, ", Auth: %s",
                                                       defauth);
        return;
    }
    ...
    // Auth algorithm
    buf_offset += sprintf(buf + buf_offset, ", Auth: ");
    for (i = 0; i < count; i++) {
        if (i != 0) {
            // Add a space between each supported cipher
            buf_offset += sprintf(buf + buf_offset, " ");
        }
        buf_offset += wt_print_auth(buf + buf_offset, data + 2 + (i * 4));
    }
}

// Print RSN information to buf.
// Actual processing is done in print_rsn_ie.
// This function just called print_rsn_ie with some default values
static void wt_print_rsn(char *buf, uint8_t len, const uint8_t *data)
{
    wt_print_rsn_ie(buf, "Unknown", "Unknown", len, data);
}
        ...
        if (ie[0] == 0 && ie[1] >= 0 && ie[1] <= 32) {
            ...
            for (i = 0; i < len; i++) {
                if (isprint(data[i]) && data[i] != ' ' && data[i] != '\\') {
                    sindex += sprintf(entry->ssid + sindex, "%c", data[i]);
                } else if (data[i] == ' ' && (i != 0 && i != len -1)) {
                    sindex = sprintf(entry->ssid + sindex, " ");
                } else {
                    sindex = sprintf(entry->ssid + sindex, "\\x%.2x", data[i]);
                }
            }
        } else if (ie[0] == 61) {
            // HT OP
            ...
            // HT OP
            data = ie + 2;
            wt_print_rsn(entry->enc_buf, ie[1], data);
        }
        ielen -= ie[1] + 2;
        ie += ie[1] + 2;
    }
